...
// this shim first. This walks the page's top-level Fragment children,
// swaps the RegisterCta element for its already-awaited output, and leaves
...
async function resolveRegisterCta(page: React.ReactElement): Promise<React.ReactElement> {
  ...
  // loads after `createClientMock` above has actually been initialized;
  // a static import here would get hoisted ahead of that `const` and throw
  // "Cannot access 'createClientMock' before initialization" the moment
  // the mock factory below runs. Matches the same specifier the page module
...
describe("YouthPage", () => {
  it("renders the title, theme, coordinator, and full multi-day schedule", async () => {
    ...
    const Page = await resolveRegisterCta(await YouthPage({ params: Promise.resolve({ locale: "en" }) }));
    ...
    expect(screen.getByText(youthProgram.history)).toBeInTheDocument();
