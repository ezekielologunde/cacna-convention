...
export function PageHero({
  ...
  return (
    ...
    >
      ...
      />
      {variant === "split" && stat ? (
        <div className="relative mx-auto flex max-w-6xl flex-wrap items-end justify-between gap-8 2xl:max-w-[100rem]">
          <div className="max-w-xl">
            ...
            <h1 className="mt-3 font-display text-4xl text-white sm:text-6xl lg:text-7xl">{title}</h1>
            ...
            {subtitle && <p className="mt-3 max-w-[52ch] text-lg text-white">{subtitle}</p>}
          </div>
        ...
        <div className="relative mx-auto max-w-5xl text-center 2xl:max-w-7xl">
          {Icon && (
            <span className="mb-4 inline-grid h-12 w-12 place-items-center rounded-2xl bg-white/12 text-white backdrop-blur-sm">
              <Icon aria-hidden={true} size={22} strokeWidth={2} />
            </span>
          ...
          <h1 className="mt-3 text-balance font-display text-5xl text-white sm:text-7xl lg:text-8xl">{title}</h1>
          {subtitle && <p className="mx-auto mt-3 max-w-[56ch] text-lg text-white">{subtitle}</p>}
        </div>
      )}
    </section>
  );
