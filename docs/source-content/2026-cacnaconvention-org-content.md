...
- **CAC Worldwide, correctly identified this time**: `cac-worldwide.org` (not `cachqtrsonline.org`, which is broken per the entry above) — a real, working official site. Nigeria HQ, ~5M members worldwide, current leadership named on the homepage (Pastor Dr. Henry Ojo, Pastor James Deke Onagwa, Prophet Richard Adebayo Kolawole, Pastor Dr. Emmanuel Olujide Babajide).
- **CAC News**: `cacworldnews.com` — a real, working news platform for the wider CAC denomination (articles, leader interviews, church profiles).
...
**Correction (2026-07-17), still later the same day:** the site owner directly confirmed that `cac-worldwide.org` is **not** the organization's actual worldwide site, despite the content (leadership names, membership figures, "One Fold One Shepherd" tagline match) looking plausible on inspection. This project's own verification isn't a substitute for the owner's own knowledge of which sites are actually affiliated. Removed the "CAC Worldwide" entry from `lib/content/external-resources.ts` entirely rather than leave it pointing somewhere unconfirmed; "CAC News" (`cacworldnews.com`) wasn't flagged as wrong and stays linked. Re-add CAC Worldwide once the correct URL is known — do not re-link `cac-worldwide.org`.

**Update (2026-07-17), later still:** the site owner pointed to `cacnorthamerica.com` again, this time to be added as a linked resource (it was already noted above as a real, verified sibling site, but not previously linked from External Resources). Confirmed live: a real site titled "CAC North America," currently showing the 2026 CACNA Annual Convention as its homepage content. Added as "CAC North America" in `lib/content/external-resources.ts`, listed ahead of CAC News since it's the parent organization's own site rather than a third-party news platform.
...
## 2027 convention dates + registration open date (2026-07-17)

The site owner provided two more facts directly: the **2027 convention is July 12–17**, and **registration opens in October**. Neither is independently published anywhere yet (same "owner-provided, not source-derived" treatment as the earlier facts in this doc).

One inference made explicit here: the owner said "October" without a year. The 2026 convention's own registration pricing table (see "Registration pricing" section above) shows its earliest tier ran **Oct 1, 2025 – Jan 31, 2026** — i.e., registration opened roughly 9 months before the July convention, in the *prior* calendar year. Applying that same lead time to a July 2027 convention puts registration opening in **October 2026** (a few months from today, 2026-07-17), not October 2027 (which would be after the convention already happened, and so can't be what was meant). Updated the homepage kicker (now shows the real dates instead of just "2027 · CAC Village, USA") and both "registration not open yet" strings (`Home.registrationComingSoon`, `Register.notOpenYet`) accordingly. If the intended year was actually 2027, this needs a correction.

## Second re-crawl: payment options, worldwide leadership, youth history, bios, name fix (2026-07-20)

A fresh re-crawl of both sites found several more real items, all added this pass:

- **Payment Options** (`cacnaconvention.org/payment-options/`) — not linked from any visible nav menu, only discoverable via the site's pages sitemap, but real, current content: Zelle (`cacnaconvention@gmail.com`), Check (Chase Bank, "CACNA CONVENTION," Account 823936908), Credit/Debit Card (4% surcharge). Now `lib/content/payment-options.ts`, rendered on the Register page.
- **CAC Nigeria & Overseas worldwide leadership** — the parent organization's executive roster: Pastor S. O. Oladele (President), Pastor E. O. Odejobi (Gen. Superintendent), Prophet H. O. Oladeji (Gen. Evangelist), Pastor E. E. Mapur (Gen. Secretary), Pastor J. F. Omitinde (Finance Director), Pastor C. S. Fasuyi (Missions Director). Real individual headshots found and downloaded for the first three (`public/photos/people/oladele.jpg`, `odejobi.jpg`, `oladeji.jpg`); no individual photos found for the other three. Now `lib/content/worldwide-leadership.ts`, a new About-page section between Leadership and Committee.
- **Youth Department history** (`cacnorthamerica.com/youth-young-adult/`) — one paragraph on the organization's founding-era youth fellowships, now in `lib/content/youth-program.ts` (`history` field), rendered on the Youth page above the schedule.
- **Leadership bios for Agbeja and Oluwatimilehin** — verified against `cacnorthamerica.com/pastor-agbeja` and `cacnorthamerica.com/pastor-john-oluwatimilehin-profile/` respectively (birthplace, ordination year, ministry history). Added as a `bio` field on their existing Leadership entries only — **not** extended to the other 3 names the site owner originally selected (Adetobi, Dada, an "Abiara interview") because (a) no substantial bio/interview text was found for them, and (b) a standalone "5 superintendent bios" section would duplicate cacnorthamerica.com's own 24-person zone/DCC directory, which a recent About-page rebuild deliberately trimmed to a blurb + link-out. Confirmed this narrower scope with the site owner before implementing.
- **Committee page Secretary name correction**: the Convention Committee/Contacts page's "Pastor Timothy Famojuro" was wrong — corrected to "Pastor Oluwagbemiga Famojuro, D.Min.," matching this project's own `committee.ts` and independently corroborated by cacnorthamerica.com's superintendent roster ("Pastor Gbenga Famojuro, D.Min." at the same phone number — Gbenga being the common short form of Oluwagbemiga).

## Third re-crawl: section-by-section sweep via 4 parallel research agents (2026-07-20)

At the user's request to "drill down the old site every section," four research agents swept cacnaconvention.org's full sitemap, cacnorthamerica.com, all 7 sub-conference pages, and the remaining Leadership members' bio pages. Most of what surfaced either duplicated existing captures or turned out to be unusable (broken/duplicated source-site content, dead links, leftover theme-demo pages) — full agent findings aren't reproduced here, only what got added:

- **Bios for 5 more Worldwide Leadership members** (Oladele, Odejobi, Oladeji, Mapur, Omitinde) — verified directly against each person's individual profile page on cacnorthamerica.com. Fasuyi's profile page's biography section is unfinished/cut off on the source site, so he has no bio.
- **CAC Nigeria & Overseas worldwide history** — condensed from the printed 2026 convention program's dedicated history page (downloaded and extracted via `pdftotext`, verified directly): 1918 founding as "Faith Tabernacle," the 1930 Ilesa revival under Apostle Joseph Ayo Babalola, several name changes before registering as "Christ Apostolic Church" in 1943. Added alongside the Worldwide Leadership section — distinct from CACNA's own regional `history.ts` (1976 Brooklyn founding) and not covered by cacnorthamerica.com's own /about page either.
- **Business Group founding** — a real excerpt from the 2026 program's President's Message: CACBGF was formally inaugurated May 4, 2026 at Ikeji-Arakeji, Osun State; explicitly not a policy-making body.
- **Convention Chairman transition** — Pastor David Adenodi's welcome address in the same program announces the completion of his tenure as Chairman of Conventions and Conferences after serving since 2000, quoting 2 Timothy 4:7. Added to the News page.
- **Good Women donation fact** — from the Convention Chairman's welcome address: the department gave $40,000 (2025) and $50,000 (2026) toward the convention's free-food initiative.
- **Save the Date table** — a 2027–2030 upcoming-convention-dates table found on the Good Women Association's own conference page in the program (convention-wide, not GWA-specific). 2027's dates independently match this project's own owner-provided registration data.

**Explicitly not added, with reasons:**
- **Adenodi/Olawale/Adelani individual bios** — their cacnorthamerica.com profile pages exist and return HTTP 200 with the correct name in the title, but the body biography text on all three is a verbatim copy of Pastor Agbeja's bio (confirmed via raw-HTML name-occurrence counts, not just AI-summarized fetches — a genuine source-site content bug, not a caching artifact). Publishing this would attribute Agbeja's life story to the wrong people.
- **10 candidate gallery photos** from cacnorthamerica.com's own `/about-2/gallery/` — downloaded to a scratchpad and visually reviewed one by one. Several are clearly from a Bible Institute graduation/convocation (caps, gowns, academic hoods), not the annual convention; none have captions or dates anywhere on the source site. Mixing them into the existing, explicitly-labeled "2025 Convention" gallery album would misattribute them.
